package com.amsterly.lovecoder.lovecoder.db;
...
public class DBManage {
        ...
        if (sDBManage == null) {
            synchronized (DBHelper.class) {
                if (sDBManage == null) {
                    ...
                    sDBHelper = DBHelper.getInstance(App.sContext);
                }
            }
        }
        return sDBManage;
    ...
    public void copyCitysToDB() {
        ...
        TaskExecutor.executeTask(new Runnable() {
            @Override
            public void run() {
                String citys = FileUtil.assetFile2String("cityList.txt", App.sContext);
                CityEntry cityEntry = App.getGson().fromJson(citys, CityEntry.class);
